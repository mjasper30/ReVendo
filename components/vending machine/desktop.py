...
# url_update_light_status = "http://192.168.43.85:3001/update_light_status"
# url_update_bin_status = "http://192.168.43.85:3001/getStorageStatus"
...
words = ["Place your plastic bottle", "Processing...",
         "Accepted", "Rejected", "Heavy Object"]
            ...
            for image_file in [f for f in os.listdir(IMAGE_DIR) if f.endswith(('.jpg', '.png', '.jpeg'))]:
                ...
                for result in results.boxes.data.tolist():
                    ...
                    if score > threshold:
                        ...
                        class_label = results.names[int(
                            class_id)].upper() + "  {:.2f}".format(score)
                ...
                while distance_status:
                    ...
                    if dist < 28.0:
                        ...
                        if timer_counter == 80:
                            print(
                                "Invalid there is no bottle pass inside the machine")
                            # Reset servo angle
                            servo.angle = 0
                            break
                ...
                data = {'rfid': global_rfid, 'height': height_cm, 'weight': int(
                    weight), 'size': size_of_object, 'no_object': 'no' if not results.boxes.data.tolist() or size_of_object == "Heavy Object" else 'yes'}
...
def claim_points():
    ...
    data = {'status': "off"}
    response_light_update_data = requests.put(
        url_update_light_status, data=data)
...
def process_plastic_bottles():
    ...
    totalplasticbottle_text.insert(
        ...
        0, str(total_large + total_medium + total_small))
    totalplasticbottle_text.config(state='disabled')
...
def check_points_scan_rfid_page():
    ...
    balance_label = Label(app, text="Balance", font=(
