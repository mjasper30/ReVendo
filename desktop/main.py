...
try:
    while True:
        ...
        if response_check_rfid.status_code == 200 and "yes" in response_check_rfid.text:
            ...
            model = YOLO(os.path.join('.', 'model', 'best5.pt'))
            ...
            # API send data
            data = {'rfid': rfid_uid, 'height': height_cm, 'weight': int(weight), 'size': size_of_object, 'no_object': 'no' if not results.boxes.data.tolist() else 'yes'}
            ...
            if choice.lower() != "y":
                ...
                # API update points
                data_to_update_points = {'rfid': rfid_uid, 'additionalPoints': total_points}
